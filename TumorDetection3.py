...
model = keras.Sequential([
    ...
    layers.Conv2D(128, (2, 2), activation='relu', padding='same', input_shape=(128, 128, 3)),
    ...
    # Sesto blocco di Convoluzione
    layers.Conv2D(32, (2, 2), activation='relu', padding='same'),
    layers.BatchNormalization(),
    layers.MaxPooling2D((2, 2)),
    layers.Dropout(0.1),
    
    # Settimo blocco di Convoluzione
    ...
    layers.Conv2D(8, (2, 2), activation='relu', padding='same'),
    layers.BatchNormalization(),
    ...
    layers.Dropout(0.2),
    layers.Dense(4, activation='softmax')  # Classi: GliomaTumor, Meningioma Tumor, Pituitary Tumor, Healthy
])
print(model.summary())
...
model.save('/content/drive/MyDrive/ProjectCNNsBrainTumor/tumor_classification_ModelProva1DropOut01.keras')
