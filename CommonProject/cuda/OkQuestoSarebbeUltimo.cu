...
__global__ void applyMultiBandGainKernel(float* __restrict__ real, float* __restrict__ imag, const int numSamples) {
    // Calcola l'indice base per il thread corrente
    const int tid = threadIdx.x;
    const int totalThreads = blockDim.x * gridDim.x;
    const int baseIdx = blockIdx.x * blockDim.x + tid;
    
    // Ogni thread processa ELEMENTS_PER_THREAD elementi consecutivi
    #pragma unroll
    for (int i = 0; i < ELEMENTS_PER_THREAD; i++) {
        const int idx = baseIdx + i * totalThreads;
        if (idx < numSamples) {
            float gain = (idx < d_bandLimits[0]) ? d_gains[0] : 
                        (idx < d_bandLimits[1]) ? d_gains[1] : d_gains[2];
...
__global__ void applyMultiBandGainKernel(float* __restrict__ real, float* __restrict__ imag, const int numSamples) {
    ...
    #pragma unroll
    for (int i = 0; i < ELEMENTS_PER_THREAD; i++) {
        const int globalIdx = baseIdx + i * totalThreads;
        const int sharedIdx = tid + i * blockDim.x;
...
int main(int argc, char* argv[]) {
    ...
    const char* outputFile = "/content/drive/MyDrive/Colab Notebooks/fullSong22222222222222222222.wav";
    SF_INFO sfInfo{};
