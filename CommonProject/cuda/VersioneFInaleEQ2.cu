...
__global__ void applyMultiBandGainKernelOptimized(float* __restrict__ real, float* __restrict__ imag, const int numSamples) {
    ...
    // Calcolo del gain usando operatori logici
    const bool isLowBand = (globalIdx < d_bandLimits[0] / 4);
    const bool isMidBand = (globalIdx < d_bandLimits[1] / 4);
    const float threadGain = (isLowBand * d_gains[0]) + 
                           (!isLowBand && isMidBand) * d_gains[1] + 
                           (!isMidBand) * d_gains[2];

    // Creiamo un float4 con lo stesso gain per tutte le componenti
    float4 gain = make_float4(threadGain, threadGain, threadGain, threadGain);

    // Resto del kernel rimane identico...
    #pragma unroll
...
void applyCudaEqualizer(float* real, float* imag, int numSamples, int sampleRate) {
    ...
    const int numBlocks = (numSamples/4 + threadsPerBlock - 1) / threadsPerBlock;
