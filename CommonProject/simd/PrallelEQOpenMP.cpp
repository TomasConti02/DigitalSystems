/*
    ...
    g++ -o ParallelEQ ParallelEQ.cpp -lsndfile -lfftw3f -fopenmp 
...
// parallel EQ
u_int64_t applyParallelEqualizerFloatMP(float* real, float* imag, int numSamples, int sampleRate) {
...
int main(int argc, char* argv[]) {
    ...
    const char* outputFile = "/home/tomas/Desktop/fullSongEQQQQQQ2.wav";
    SF_INFO sfInfo;
    ...
    unsigned long long parallelTimeMP = applyParallelEqualizerFloatMP(real, imag, numSamples / 2, SAMPLE_RATE);
   ...
   std::cout << "\tSPEEDUP SIMD => " << (float)scalarTime / (float) parallelTime << "\n" << std::endl;
   std::cout << "\tSPEEDUP SIMD+OpenMP => " << (float)scalarTime / (float) parallelTimeMP << "\n" << std::endl;
   return 0;
}
