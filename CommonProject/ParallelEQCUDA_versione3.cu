...
__global__ void applyMultiBandGainKernelMultiDataforThread(float* real, float* imag, int numSamples) {
    // Memoria condivisa per caricare i dati
    __shared__ float shared_real[BLOCK_SIZE];
    __shared__ float shared_imag[BLOCK_SIZE];
    
    int globalIdx = blockIdx.x * blockDim.x + threadIdx.x;

    // Ogni thread processa più campioni
    for (int idx = globalIdx; idx < numSamples; idx += gridDim.x * blockDim.x) {
        // Carica i dati nella memoria condivisa
        if (idx < numSamples) {
            shared_real[threadIdx.x] = real[idx];
            shared_imag[threadIdx.x] = imag[idx];
        }
        __syncthreads(); // Sincronizza per garantire che i dati siano pronti

        // Calcola il guadagno basato sulla banda
        if (idx < numSamples) {
            float gain;
            ...
            // Applica il guadagno ai dati condivisi
            shared_real[threadIdx.x] *= gain;
            shared_imag[threadIdx.x] *= gain;
        }
        __syncthreads(); // Sincronizza prima di scrivere nella memoria globale

        // Scrivi i risultati nella memoria globale
        if (idx < numSamples) {
...
__global__ void applyMultiBandGainKernelGlobal(float* real, float* imag, int numSamples) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
void applyCudaEqualizer(float* real, float* imag, int numSamples, int sampleRate) {
    ...
    // Esecuzione del kernel ottimizzat
    /*
    auto start = std::chrono::high_resolution_clock::now();
    applyMultiBandGainKernel<<<gridDim, blockDim>>>(d_real, d_imag, numSamples);
    cudaDeviceSynchronize();
    auto end = std::chrono::high_resolution_clock::now();
    std::cout << "CUDA execution time Pralellel Shared: " << std::chrono::duration_cast<std::chrono::microseconds>(end - start).count() << " µs\n";
     start = std::chrono::high_resolution_clock::now();
    applyMultiBandGainKernelGlobal<<<gridDim, blockDim>>>(d_real, d_imag, numSamples);
    cudaDeviceSynchronize();
     end = std::chrono::high_resolution_clock::now();
    std::cout << "CUDA execution time Parallel GLobal: " << std::chrono::duration_cast<std::chrono::microseconds>(end - start).count() << " µs\n";
     start = std::chrono::high_resolution_clock::now();
    applyMultiBandGainKernel<<<1, 1>>>(d_real, d_imag, numSamples);
    cudaDeviceSynchronize();
     end = std::chrono::high_resolution_clock::now();
    std::cout << "CUDA execution time Sequential " << std::chrono::duration_cast<std::chrono::microseconds>(end - start).count() << " µs\n";
    */
    auto start = std::chrono::high_resolution_clock::now();
    applyMultiBandGainKernelMultiDataforThread<<<gridDim, blockDim>>>(d_real, d_imag, numSamples);
    cudaDeviceSynchronize();
    auto end = std::chrono::high_resolution_clock::now();
    std::cout << "CUDA execution time Sequential " << std::chrono::duration_cast<std::chrono::microseconds>(end - start).count() << " µs\n";
...
int main(int argc, char* argv[]) {
    ...
    const char* outputFile = "/content/drive/MyDrive/Colab Notebooks/fullSong8.wav";
    SF_INFO sfInfo{};
