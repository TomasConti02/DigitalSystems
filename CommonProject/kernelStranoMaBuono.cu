...
#define WAVES_PER_BLOCK 2  // Aumentiamo l'occupancy

__global__ void applyMultiBandGainKernelOptimized(
    float* __restrict__ real, 
    float* __restrict__ imag, 
    const int numSamples
) {
    // Cast a vector types per un accesso efficiente
    float4* realVec = reinterpret_cast<float4*>(real);
    float4* imagVec = reinterpret_cast<float4*>(imag);

    // Memoria condivisa con padding per evitare bank conflicts
    ...
    // Calcolo dei limiti di banda in vettori (float4)
    const int bandLimit0 = d_bandLimits[0] / 4;
    const int bandLimit1 = d_bandLimits[1] / 4;

    // Pre-caricamento e applicazione guadagno
    if (globalIdx < numSamples / 4) {
        float4 rData = realVec[globalIdx];
        float4 iData = imagVec[globalIdx];

        // Calcolo guadagni per i 4 elementi
        float4 gain;
        gain.x = (globalIdx < bandLimit0) ? d_gains[0] : 
                 (globalIdx < bandLimit1) ? d_gains[1] : d_gains[2];
        gain.y = gain.x;
        gain.z = gain.x;
        gain.w = gain.x;

        // Applicazione guadagno
        rData.x *= gain.x; rData.y *= gain.y; 
        rData.z *= gain.z; rData.w *= gain.w;
        iData.x *= gain.x; iData.y *= gain.y; 
        iData.z *= gain.z; iData.w *= gain.w;

        // Scrittura nella memoria condivisa
        sharedReal[tid] = rData;
        sharedImag[tid] = iData;
    }
    ...
    if (globalIdx < numSamples / 4) {
        realVec[globalIdx] = sharedReal[tid];
        imagVec[globalIdx] = sharedImag[tid];
    }
}

// Funzione helper per il lancio del kernel
void launchMultiBandGainKernel(
    float* d_real, 
    float* d_imag, 
    int numSamples
) {
    const int numThreads = BLOCK_SIZE;
    const int numBlocks = (numSamples + (BLOCK_SIZE * ELEMENTS_PER_THREAD - 1)) / 
                         (BLOCK_SIZE * ELEMENTS_PER_THREAD);

    // Configurazione della L1 cache per privilegiare la bandwidth
    cudaFuncSetCacheConfig(applyMultiBandGainKernelOptimized, cudaFuncCachePreferL1);

    applyMultiBandGainKernelOptimized<<<numBlocks, numThreads>>>(
        d_real, d_imag, numSamples
    );
}
void applyCudaEqualizer(float* real, float* imag, int numSamples, int sampleRate) {
    ...
    launchMultiBandGainKernel(d_real, d_imag, numSamples);
...
int main(int argc, char* argv[]) {
    ...
    const char* outputFile = "/content/drive/MyDrive/Colab Notebooks/fullSong71.wav";
    SF_INFO sfInfo{};
